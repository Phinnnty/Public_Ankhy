    ...
    class DistanceMatrix extends Array {
        ...
        Append(newVector) {
            ...
            loop standard.Length
                newRow.Push(Distance(newVector, standard[A_Index]) + Min(newRow[A_Index], lastRow[A_Index + 1]))
        }

        TraceBack() {
            i := this.Length - 1 , j := this[1].Length - 1 , count := 1
            while i > 1 && j > 1 {
                minValue := Min(this[i][j], this[i][j - 1])
                if (this[i - 1][j] < minValue)
                    minValue := this[i - 1][j]
                if (minValue = this[i][j])
                    i--, j--
                else if (minValue = this[i][j - 1])
                    j--
                else
                    i--
                count++
